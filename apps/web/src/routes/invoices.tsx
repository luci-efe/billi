        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2 min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">{formatDate(transaction.occurredAt)}</span>
            </div>
            <div>
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,240px)_1fr]">
          <div className="rounded-lg border border-border bg-muted/40 p-3">
            <div className="mb-2 flex items-center gap-2 text-xs font-medium text-foreground">
              <Upload className="h-3.5 w-3.5 text-indigo-600 dark:text-indigo-300" />
              Subir comprobante
            </div>
          ...
          <div className="rounded-lg border border-border bg-muted/40 p-3">
            <div className="mb-2 flex items-center gap-2 text-xs font-medium text-foreground">
              <FileText className="h-3.5 w-3.5 text-indigo-600 dark:text-indigo-300" />
              Archivos vinculados
            </div>
            {error ? (
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 lg:pb-0">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-foreground">Facturas y Recibos</h2>
      ...
      {isLoading && transactions.length === 0 ? (
        <Card>
          <CardContent className="py-10 text-sm text-muted-foreground">Cargando comprobantes…</CardContent>
        </Card>
      ) : error ? (
        <Card>
          <CardContent className="py-10 text-sm text-rose-600 dark:text-rose-400">{error.message}</CardContent>
        </Card>
      ) : transactions.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-300">
              <Receipt className="h-8 w-8" />
            </div>
            <div className="space-y-2">
              <p className="text-lg font-semibold text-foreground">Aún no tienes movimientos para documentar</p>
